...
describe("isXaiChatModel", () => {
    it("treats xAI chat models as the happy path without an allowlist", () => {
        assert.equal(isXaiChatModel({ id: "grok-4.7", provider: "xai" }), true);
        assert.equal(isXaiChatModel({ id: "xai/grok-4.7", provider: "xai" }), true);
    ...
    it("excludes image-generation models and other providers", () => {
        ...
        assert.equal(isXaiChatModel({ id: "grok-4.7", provider: "openai" }), false);
